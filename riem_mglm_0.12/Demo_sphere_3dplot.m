...
for iter = 1:1
    ...
    for i = 1:npivots
        for i2 = 1:num_plt
            plot33r(expmap_sphere(Yp(:,1), i2 / num_plt * V(:,i)))
        end
        quiver3(Yp(1,1),Yp(2,1),Yp(3,1),V(1,i),V(2,i),V(3,i))
    end
    ...
    for i = 1:mse_iter
        ...
        plot3(pls(1,1),pls(2,1),pls(3,1),'b*')
        ...
        plot3(phb(1,1),phb(2,1),phb(3,1),'g*')
...
%         end
%         plot3(ptk(1,1),ptk(2,1),ptk(3,1),'m*')

        for j = 1:npivots
            for i2 = 1:num_plt
                plot33b(expmap_sphere(pls, i2 / num_plt * Vls(:,j)))
                plot33g(expmap_sphere(phb, i2 / num_plt * Vhb(:,j)))
%                 plot33m(expmap_sphere(ptk, i2 / num_plt * Vtk(:,j)))
            end
        end
        
%         quiver33(pls,phb,ptk,Vls,Vhb,Vtk,npivots)
    ...
    fprintf('mse_p_ls %f mse_p_hb %f mse_p_tk %f\n', mse_p_ls/mse_iter, mse_p_hb/mse_iter, mse_p_tk/mse_iter)
    fprintf('mse_v_ls %f mse_v_hb %f mse_v_tk %f\n', mse_v_ls/mse_iter, mse_v_hb/mse_iter, mse_v_tk/mse_iter)
end

function plot33(point)
    plot3(point(1,1),point(2,1),point(3,1),'o')
end

function plot33r(point)
    plot3(point(1,1),point(2,1),point(3,1),'ro')
end

function plot33b(point)
    plot3(point(1,1),point(2,1),point(3,1),'bo')
end

function plot33g(point)
    plot3(point(1,1),point(2,1),point(3,1),'go')
end

function plot33m(point)
    plot3(point(1,1),point(2,1),point(3,1),'mo')
end

function quiver33(point1,point2,point3,V1,V2,V3,npivots)
    for i = 1:npivots
        quiver3(point1(1,1),point1(2,1),point1(3,1),V1(1,i),V1(2,i),V1(3,i))
        quiver3(point2(1,1),point2(2,1),point2(3,1),V2(1,i),V2(2,i),V2(3,i))
        quiver3(point3(1,1),point3(2,1),point3(3,1),V3(1,i),V3(2,i),V3(3,i))
    end
end