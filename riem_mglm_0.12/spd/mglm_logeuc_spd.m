...
%   $ Revision: 0.12 $ 
    ...
    L = Yv/Xc;% Peform MGLM, B/A는 xA=B의 Least squares solution 을 구함. 
    % L은 6*npivots 행렬, Yv는 6*ndata, Xc는 npivots*ndata, L = YvXc'(XcXc')^(-1)
    logYv_hat = L*Xc; % Prediction.
    
    %Transport Y^{\wr} back to TpM
    V_hat = invembeddingR6_vecs(p,logYv_hat); % V_hat은 VXc
