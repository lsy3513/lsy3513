...
X1 = [0:0.25:1 0:0.25:1 0:0.25:1 0:0.25:1];    
...
isample = 1;
for i = 1:(npairs * (1 - p_outliers))
    for j = 1:size(Y,3)
        Ysample(:,:,isample) = addnoise_spd(Y(:,:,j),noise1,max_noise);
        isample = isample + 1;
    end
    Xsample =[Xsample X];
end
for i = ((npairs * (1 - p_outliers)) + 1):npairs
    for j = 1:size(Y,3)
        Ysample(:,:,isample) = addnoise_spd(Y(:,:,j),noise2,max_noise);
        isample = isample + 1;
    end
    Xsample =[Xsample X];
end
