...
function q = addnoise_spd_t(p, nu, err_scale, maxerr)
    n = size(p,1);
    err_size = randt_f(n,nu, err_scale,maxerr);
    x = randpoint(n);
...
% sample from f(x) ~ (1+x^2/nu)^(-(nu+1)/2)  * x^(n-1) (0 < x < maxerr)
%(n=dimY=3)
function x = randt_f(n,nu,c,maxerr) 
    while true
        while true
            x = abs(trnd(nu))*c;
            if x < maxerr
