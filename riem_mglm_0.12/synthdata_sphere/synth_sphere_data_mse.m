...
while true
    ...
    for i = 2:(npivots+1)
        Yp(:,i) = addnoise_sphere(Yp(:,1),0.5,2);
    end
...
while true
    ...
    for i = 2:(npivots+1)
        Yp_outlier(:,i) = addnoise_sphere(Yp_outlier(:,1),0.5,2);
    end
...
for k = 1:mse_iter
    for i = 1:(npairs - noutliers)
        Ysample(:,i,k) = addnoise_sphere(Y_0(:,i),noise_size,max_noise_size);
    end
    for i = (npairs - noutliers+1):npairs
        Ysample(:,i,k) = addnoise_sphere(Y_0_outlier(:,i),noise_size,max_noise_size);
    end
end
% for k = 1:mse_iter
%     for i = 1:(npairs - noutliers)
%         Ysample(:,i,k) = addnoise_sphere_old(Y_0(:,i),noise_size,max_noise_size);
%     end
%     for i = (npairs - noutliers+1):npairs
%         Ysample(:,i,k) = addnoise_sphere_old(Y_0_outlier(:,i),noise_size,max_noise_size);
%     end
% end 
